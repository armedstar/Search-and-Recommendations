...
(function() {
  ...
  jingo.init = function(config) {
    ...
    paramCheck('jingo.init', ['verbosity', 'repos', 'timeout'], config);
  };
  ...
  jingo.declare = function(config) {
    paramCheck('jingo.declare', ['require', 'name', 'as'], config);
  ...
  jingo.anonymous = function(config) {
    paramCheck('jingo.anonymous', ['require', 'exec'], config);
  ...
  jingo.Module = function(counter) {
    ...
    return function(config) {
      ...
      var normalizeRepo = function(repo) {
        if(!repo || /\/$/.exec(repo)) {
          return repo;
        }
        return repo + '/';
       ...
       * @type string
       */
      this.repo = normalizeRepo(config.repo);
  ...
  jingo.Manager = function() {
     ...
     *  declaration.
     * @param {string} defaultRepo This parameter holds the repository URL that the specified 
     *  dependency declaration should default to if it has no repo prefix of its own.
     ...
     */
    var parseDependency = function(dependency, defaultRepo) {
      var parts = dependency.split(':');
      if(parts.length < 2) {
        return {
          repo: defaultRepo || self.repos.main,
          name: parts[0]
        };
      }
      return {
    ...
    this.declare = function(name, config) {
      ...
      jingo.iterators.each(module.dependencies, function(index, dependency) {
        dependency = parseDependency(dependency, module.repo);
